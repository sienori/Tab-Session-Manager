...
function sessionsHTML(info) {
  ...
  return `<div id=${info.id} class="session" data-tag=${dataTag} data-date="${
    info.date
  }" data-name="${sanitaize.encode(info.sessionName)}">
...
function showSessions(sessions, isInit = true) {
  ...
  if (isInit) {
    ...
    sessionsArea.insertAdjacentHTML(
      "afterbegin",
      `<div class="noSessionLabel hidden">${Labels.noSessionLabel}</div>`
    );
  }
  ...
  if (detail.classList.contains("hidden") || overwrite) {
    ...
    for (let win in session.windows) {
      ...
      for (let tab of sortedTabs) {
        ...
        const tabHtml = `<li class="tabContainer ${overwrite ? "" : "hidden"}">
                        <div class=fav style="background-image:url(${sanitaize.encode(
                          tabFavIconUrl
                        )})"></div>
...
function replaseImageUrl(url, sessionId, win) {
  const favElements = document
    .getElementById(sessionId)
    .getElementsByClassName("windowContainer")
    [win - 1].getElementsByClassName("fav");
...
document.addEventListener("click", async function(e) {
  ...
  switch (e.target.id) {
    ...
    case "donate":
      const url =
        "https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&no_shipping=1&business=sienori.firefox@gmail.com&item_name=Tab Session Manager - Donation";
      openUrl(url);
      break;
  }
